...
then
  echo '✖ ERROR: These documentation pages need front matter. See <URL> for adding front matter to documentation.' >&2
  echo
  ((ERRORCODE++))
fi
...
then
  ...
  echo '  <URL>: Documentation standards for work with directories and files'
  echo
  ((ERRORCODE++))
fi
